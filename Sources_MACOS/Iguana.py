...
class Pappl(QtWidgets.QWidget, interface_ui.Ui_Form):
    ...
    def colorations(self):
        ...
        if (self.time.value()==0):
            command=dir_path+"/clingo " +str(nbColor)+" "+dir_path +"/optimizationComponent.lp "+input+" --opt-mode=optN --enum-mode=cautious --parallel-mode=2  > "+ os.path.splitext(input)[0] +"-colorations.txt"
        else:
            command=dir_path+"/clingo " +str(nbColor)+" "+dir_path +"/optimizationComponent.lp "+input+" --time-limit="+str(self.time.value())+" --opt-mode=optN --enum-mode=cautious > "+ os.path.splitext(input)[0] +"-colorations.txt"
        print(command)
